...
def get_highlight(db_row):
    ...
    if "concord_set" in db_row and db_row["concord_set"]:
        ...
        con_list = json.loads(db_row["concord_set"])
        if "con" in con_list:
            for con in con_list["con"]:
...
def get_query_parts(criteria, page, page_size, is_stat, book):
    ...
    for crit in criteria:
        if is_first:
            if is_stat:
                cursor_string += ("SELECT Book.title, Book.author,"
                                  " COUNT(DISTINCT {}.hash) AS total FROM Verse {}".format(
                                      curr_char, curr_char))
            else:
                cursor_string += ("SELECT DISTINCT Book.title, {}.hypertext, {}.text,"
        ...
        verse_match = is_verse(crit.get("english", {}).get("words", [""])[0])
        if verse_match:
            verse_parts = verse_match.group().split(":")
            book = re.match("[-.0-9]*[ ]*[a-zA-Z]+", verse_parts[0]).group()
            where_stmts += (' Book.title = "{}" AND {}.chapter = {}'
                            ' AND {}.verse_num = {}').format(
                                book,
                                curr_char, verse_parts[0][len(book):],
                                curr_char, verse_parts[1])
        elif (("native" in crit) and ("english" in crit) and
              (" " not in crit["english"]["words"][0])):
            ...
            join_stmts += (" JOIN Concordance {}c ON "
                           "{}c.concord_id = {}vb.concord_id").format(
                               curr_char, curr_char, curr_char)
            join_stmts += (" JOIN WordVerseBridge {}wvb ON "
                           "{}.hash = {}wvb.verse_hash").format(
                               curr_char, curr_char, curr_char)
            join_stmts += (" JOIN Word {}w ON "
                           "{}w.word_id = {}wvb.word_id").format(
                               curr_char, curr_char, curr_char)

            where_stmts += " {}c.concord_id = {}".format(
                            curr_char, crit["native"]["concords"][0])

            where_stmts += (' AND {}w.word_ci = "{}" AND ({}wvb.is_match=1').format(
                curr_char, crit["english"]["words"][0], curr_char)

            if crit["english"].get("synonym", False):
                where_stmts += " OR {}wvb.is_synonym=1".format(curr_char)

            if crit["english"].get("similar", False):
                where_stmts += " OR {}wvb.is_similar=1".format(curr_char)
